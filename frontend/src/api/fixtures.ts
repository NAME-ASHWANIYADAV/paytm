 ...
 *   • 12 dormant regulars worth ~₹8,400 of recoverable revenue
  ...
  const insights: InsightOut[] = [
    {
      ...
      severity: 'high',
      title_hi: '12 पुराने ग्राहक चार हफ़्ते से नहीं आए',
      title_en: '12 regulars have not visited in over four weeks',
      body_hi:
        'हर ग्राहक की अपनी आने की आदत के हिसाब से नापा है, 30 दिन के एक ही नियम से नहीं। इन बारह ने पिछले 90 दिन में ₹28,600 की ख़रीद की थी। वापसी की उम्मीद ₹8,400 है।',
      body_en:
        'Measured against each customer’s own visit cadence (median gap + 1.5×IQR), not a flat 30-day rule. These twelve spent ₹28,600 in the last 90 days; expected recoverable value is ₹8,400.',
      metrics: {
        dormant_count: 12,
        recoverable_paise: 840_000,
        median_gap_days: 9.4,
        days_since_last: 31,
  ...
  const actions: ActionOut[] = [
    {
      ...
      summary_hi: '12 सुस्त ग्राहकों को ₹50 का वापसी ऑफ़र भेजें',
      summary_en: 'Send a ₹50 win-back offer to 12 dormant regulars',
      params: {
        segment: 'dormant',
        offer_paise: 5000,
        channel: 'whatsapp',
        customer_count: 12,
        template: 'winback_v3',
        message_hi: 'नमस्ते {{name}} जी — बहुत दिन हो गए! इस हफ़्ते शर्मा जनरल स्टोर पर आपके लिए ₹50 की छूट।',
        valid_days: 7,
      },
      target_count: 12,
      estimated_impact: fixtureMoney(840_000),
      estimated_cost: fixtureMoney(936),
      net_expected: fixtureMoney(839064),
    ...
    },
    {
      ...
      tool_name: 'send_winback_offer',
      status: 'executed',
      summary_hi: '12 सुस्त ग्राहकों को ₹50 का ऑफ़र भेजा गया',
      summary_en: '₹50 win-back offer sent to 12 dormant regulars',
      params: { segment: 'dormant', offer_paise: 5000, channel: 'whatsapp', customer_count: 12, template: 'winback_v2' },
      target_count: 12,
      estimated_impact: fixtureMoney(780_000),
      estimated_cost: fixtureMoney(936),
      net_expected: fixtureMoney(779064),
      requested_at: istStamp(7 * 24 * 60 + 26),
      decided_at: istStamp(7 * 24 * 60 + 24),
      executed_at: istStamp(7 * 24 * 60 + 23),
      provider: 'local',
      error: '',
      result: { workflow: 'winback_offer_v2', run_id: 'run_local_8831', sent: 12, failed: 0 },
      outcomes: [
...
const NODE_SEEDS: NodeSeed[] = [
  ...
  },
  {
    ...
    attrs: { severity: 'high', impact_paise: 840_000 },
  },
  {
    id: 'mn_ins_udhaar',
    kind: 'insight',
  ...
  },
  {
    ...
    text: '₹50 win-back offer sent to 12 dormant regulars seven days ago via the local action runner.',
    attrs: { status: 'executed', target_count: 12 },
  },
  {
    id: 'mn_act_winback2',
    kind: 'action',
    label: 'Win-back offer · manzoori baaki',
    text: 'Second ₹50 win-back offer to 12 dormant regulars, waiting for the merchant to say haan.',
    attrs: { status: 'pending_approval', target_count: 12 },
  },
  {
    id: 'mn_out_recovered',
    kind: 'note',
      ...
      tool_calls: [
        {
          ...
          ok: true,
          latency_ms: 61,
          summary: '12 dormant regulars · ₹8,400 recoverable · median gap 9.4d',
        },
      ],
